...
public class StringScrollPicker extends ScrollPickerView<CharSequence> {
    ...
    public StringScrollPicker(Context context, AttributeSet attrs,
        ...
        mPaint.setShadowLayer(2, 1, 1, Color.parseColor("#80000000"));
        init(attrs);
    }
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            mBaseHeight = typedArray.getDimensionPixelSize(
                    R.styleable.StringScrollPicker_spv_base_height, mBaseHeight);
    ...
    public void drawItem(Canvas canvas, List<CharSequence> data, int position, int relative, float moveLength, float top) {
        ...
        // 设置文字大小 --end
        //StaticLayout 实现换行
        StaticLayout layout = new StaticLayout(text, 0, text.length(),
                mPaint, mMaxLineWidth, mAlignment,
        ...
        y = mBaseHeight - layout.getHeight() / 2;
